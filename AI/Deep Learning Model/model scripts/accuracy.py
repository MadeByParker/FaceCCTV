...
import scripts.detections
import scripts.geometry
...
def detect_face_correctly(image, face_bounding_box, cascade_classifier):
      ...
      if len(detections) != 1:
          return False
            ...
            detection_correct = scripts.geometry.bounding_box_iou(face_bounding_box, detection_bounding_box) > 0.5
...
def check_accuracy(image_path, bounding_box_map):
      ...
      for path in tqdm.tqdm(image_path):
            ...
            if scripts.geometry.bounding_box_iou(image_bounding_box, face_bounding_box) > 0.01:
...
def model_detect_face_correctly(image, face_bounding_box, model, configuration):
      ...
      detections = scripts.detections.FaceCCTVModel(image, model, configuration).get_face_detection()
      ...
      if len(detections) != 1:
            ...
            is_detection_correct = scripts.geometry.bounding_box_iou(face_bounding_box, detections[0].bounding_box) > 0.5
...
def check_model_accuracy(image_path, bounding_box_map, filepath=None):
      ...
      model = scripts.models.get_vgg_model(scripts.config.image_shape)
      model.load_weights(scripts.config.model_path)


      for path in tqdm.tqdm(image_path):
            image = scripts.utils.get_image(path)
            ...
            if scripts.geometry.bounding_box_iou(image_bounding_box, face_bounding_box) > 0.01:
                  ...
                  value = 1 if model_detect_face_correctly(image, face_bounding_box, model, scripts.config) else 0
                  detection_scores.append(value)
...
def main():
      ...
      dataset_path = os.path.join(scripts.config.dataset_path, dataset)

      image_path_file = scripts.utils.join(dataset_path, "training_images.txt")
      bounding_box_file = scripts.utils.join(dataset_path, "training_bounding_boxes.txt")

      image_paths = [path.strip() for path in scripts.utils.get_file_lines(image_path_file)]
      bounding_box_map = scripts.geometry.get_bounding_box_map(bounding_box_file)
